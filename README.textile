...
Software Requirements (or: "things that should be installed"):
* on Windows: if OpenAL shouldn't be installed already, please do so: http://connect.creativelabs.com/openal/Downloads/oalinst.zip
* on OS X: please install the latest XQuartz: http://xquartz.macosforge.org
...
* HiDPI/Retina support on OS X is enabled; note that the resolution specified in config.xml or config.xml.local will be doubled (e.g., the default resolution of 1280*720 will become 2560*1440); to achieve a playable framerate, you should set upscaling to "2.0" in config.xml.local (the GUI will still be rendered at the native resolution)
* due to platform specific issues, fullscreen/windowed mode switching from within the game might not work (set the fullscreen flag manually in data/config.xml.local and restart the game if this happens)
* don't dig down!
...
* Yannic Haupenthal (yhaupenthal@gmail.com)
		...
		Thank you for playing our project!
		We would love to hear your feedback (either CC us via mail or reply to the forum post)!
