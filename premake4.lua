...
project "BlocksInMotion"
	...
	if(not os.is("windows") or win_unixenv) then
		...
		add_include("/usr/include/bullet")
		
		buildoptions { "-x c++ -std=c++11 -Wall -Wno-trigraphs -Wreturn-type -Wunused-variable -funroll-loops" }
	...
	-- the same for all
	includedirs { "src/", "src/ai/", "src/audio/", "src/editor/", "src/game/", "src/map/", "src/physics/", "src/scripting/", "src/ui/" }
	...
	if(os.is("linux") or os.is("bsd") or win_unixenv) then
		...
		links { "OpenCL", "freetype", "mpg123" }
		if(not win_unixenv) then
			links { "GL", "openal", "SDL2_image", "xml2", "BulletSoftBody", "BulletDynamics", "BulletCollision", "LinearMath" }
		...
		elseif(cygwin) then
			...
			links { "opengl32", "OpenAL", "SDL2_image.dll", "xml2" }
			libdirs { "/lib/w32api" }
		...
		elseif(mingw) then
			...
			links { "opengl32", "OpenAL", "SDL2_image", "libxml2" }
			buildoptions { "`sdl2-config --cflags | sed -E 's/-Dmain=SDL_main//g'`" }
	...
	configuration "Debug"
		...
		if(win_unixenv) then
			links { "BulletSoftBody_Debug", "BulletDynamics_Debug", "BulletCollision_Debug", "LinearMath_Debug" }
		end
