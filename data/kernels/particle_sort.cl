...
kernel void compute_distances(global const float4* pos_buffer, const float4 camera_pos, global float* dist_buffer) {
	...
	if(global_id >= get_global_size(0)) return;
...
A2E_FUNC void ComparatorPrivate(global const float* dist_buffer,
								uint* keyA,
								uint* keyB,
								uint arrowDir) {
...
A2E_FUNC void ComparatorLocal(global const float* dist_buffer,
							  local uint* keyA,
							  local uint* keyB,
							  uint arrowDir) {
	const float dist_a = dist_buffer[*keyA];
...
kernel void bitonicMergeGlobal(global const float* dist_buffer,
	...
	if(global_comparatorI >= get_global_size(0)) return;
