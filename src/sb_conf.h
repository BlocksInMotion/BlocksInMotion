...
#define SB_CONF_TYPE_TO_STRING_CHECK(type_, enum_type_) (type == enum_type_) ? #type_ :
...
public:
	...
	static constexpr const char* conf_type_to_string(const CONF_TYPE type) {
		...
		return SB_CONF_TYPES(SB_CONF_TYPE_TO_STRING_CHECK) "unknown";
	}
